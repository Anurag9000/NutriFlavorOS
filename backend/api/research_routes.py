...
def catalog():
    value = get_catalog()
    return {"catalog": value, "summary": value.summary()}
...
def validate_run_config(
    ...
    experiment_ids = {item.id for item in catalog_value.experiments}
    model_ids = {item.id for item in catalog_value.models}
    if config.experiment_id not in experiment_ids:
        raise HTTPException(status_code=422, detail="Unknown experiment_id")
    if config.baseline not in model_ids and config.baseline not in {"catalog_validation"}:
        raise HTTPException(status_code=422, detail="Unknown baseline/model identifier")
    manifest = create_manifest(config)
    return {
        "valid": True,
        "manifest_preview": manifest,
        "execution": "offline_cli_only",
        "reason": "The API validates configurations but does not execute arbitrary experiments.",
    }
...
def collection(
    ...
    if readiness:
        items = [item for item in items if getattr(item, "readiness", None) and item.readiness.value == readiness]
    if risk:
        items = [item for item in items if getattr(item, "risk", None) and item.risk.value == risk]
...
def catalog_item(collection: str, item_id: str):
    try:
        ...
        raise HTTPException(status_code=404, detail="Research catalog item not found") from exc
