...
class HealthEngine:
    ...
    def score_recipe_comprehensive(self, recipe_id: str, target: NutrientTarget,
                                   user_conditions: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        for nutrient, target_val in target.micro_nutrients.items():
            actual = float(micros.get(nutrient, 0))
            # Each meal should provide ~33% of daily RDA
            micro_target_per_meal = target_val / 3
            if micro_target_per_meal > 0:
                coverage = min(1.0, actual / micro_target_per_meal)
