...
class DeviceConfig:
    ...
    def _log_device_info(self):
        ...
        if self._device_info.get("type") == "cuda":
            logger.info("=" * 60)
            logger.info("🚀 GPU ACCELERATION ENABLED")
            logger.info("=" * 60)
            logger.info(f"Device: {self._device_info.get('name')}")
            logger.info(f"CUDA Version: {self._device_info.get('cuda_version')}")
            logger.info(f"GPU Count: {self._device_info.get('count')}")
            logger.info(f"Total Memory: {self._device_info.get('memory_total', 0) / 1024**3:.2f} GB")
            logger.info("=" * 60)
        else:
            logger.warning("=" * 60)
    ...
    def get_memory_info(self) -> Dict[str, float]:
        ...
        if self.is_cuda:
            return {
                ...
                "total_gb": self._device_info.get("memory_total", 0) / 1024**3,
                "free_gb": (self._device_info.get("memory_total", 0) - torch.cuda.memory_allocated(0)) / 1024**3
            }
        return {}
...
def print_device_info():
    ...
    if _device_config.is_cuda:
        print(f"CUDA Version: {_device_config._device_info.get('cuda_version')}")
        print(f"GPU Count: {_device_config._device_info.get('count')}")

        mem_info = get_memory_info()
        print("\nMemory Information:")
        print(f"  Total: {mem_info.get('total_gb', 0.0):.2f} GB")
        print(f"  Allocated: {mem_info.get('allocated_gb', 0.0):.2f} GB")
        print(f"  Reserved: {mem_info.get('reserved_gb', 0.0):.2f} GB")
        print(f"  Free: {mem_info.get('free_gb', 0.0):.2f} GB")
