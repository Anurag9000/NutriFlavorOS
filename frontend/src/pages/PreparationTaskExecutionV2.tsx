...
interface CompletionMutationInput {
  idempotencyKey: string;
}
...
export default function PreparationTaskExecutionPage() {
  ...
  useEffect(() => {
    setTaskDrafts({});
  }, [activeHouseholdId, activeScheduleId]);
  ...
  const taskMutation = useMutation({
    mutationFn: ({
      ...
      if (eventType === "started") {
        return preparationOperationsApi.startTask(
          activeHouseholdId,
      ...
      if (eventType === "completed") {
        return preparationOperationsApi.completeTask(
          activeHouseholdId,
          overview.schedule.id,
          task.task.task_id,
          payload,
      ...
      return preparationOperationsApi.skipTask(
        activeHouseholdId,
        overview.schedule.id,
        task.task.task_id,
        payload,
  ...
  const completeSchedule = useMutation({
    mutationFn: ({ idempotencyKey }: CompletionMutationInput) => {
      if (!overview) throw new Error("Select an execution schedule");
                  ...
                  <Button
                    ...
                    onClick={() =>
                      completeSchedule.mutate({
                        idempotencyKey: eventKey("schedule-complete"),
                      })
                    }
                  >
                    <CheckCircle2 className="mr-2 h-4 w-4" />
