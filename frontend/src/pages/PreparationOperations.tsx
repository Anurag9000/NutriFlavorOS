...
const EMPTY_SCHEDULE_BUNDLE = JSON.stringify(
  {
    ...
    occurrence_set: {
      ...
      household_id: "replace-with-household-id",
...
function parseScheduleBundle(
  raw: string,
): Omit<PersistedScheduleCreateRequest, "idempotency_key"> {
  const value = JSON.parse(raw) as unknown;
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("Schedule bundle must be a JSON object");
  }
  const bundle = value as Omit<
    PersistedScheduleCreateRequest,
    "idempotency_key"
  >;
  if (!bundle.occurrence_set) {
    throw new Error(
      "Schedule bundle must contain the complete occurrence_set document",
    );
  }
  return bundle;
}
...
function TransitionButtons({
  ...
  pending: boolean;
  onTransition: (
    eventType: Exclude<PreparationScheduleEventType, "created">,
  ) => void;
}) {
  return (
    <div className="flex flex-wrap gap-2">
      {schedule.status === "draft" && role === "owner" && (
        <Button
          type="button"
          size="sm"
          disabled={pending || schedule.replay_status !== "replayable"}
      ...
      {(schedule.status === "draft" || schedule.status === "approved")
        && role === "owner" && (
          <Button
            ...
            variant="outline"
            disabled={pending}
            onClick={() => onTransition("invalidated")}
          >
            Invalidate
          </Button>
        )}
...
export default function PreparationOperationsPage() {
  ...
  const eventsQ = useQuery({
    ...
    queryFn: () =>
      preparationOperationsApi.events(
        householdId,
        expandedScheduleId as number,
      ),
  ...
  const invalidate = async () => {
    await Promise.all([
      ...
      queryClient.invalidateQueries({
        queryKey: ["preparation-operations", householdId, "schedule-events"],
      }),
    ]);
  };
  ...
  const createCalendar = useMutation({
    mutationFn: () => {
      ...
      if (
        ...
        throw new Error(
          "Horizon must be an integer from 1 to 10080 minutes",
        );
      }
    ...
    onError: (error) =>
      toast({
        title: "Calendar registration failed",
  ...
  const createSchedule = useMutation({
    mutationFn: () =>
      preparationOperationsApi.createSchedule(householdId, {
    ...
    onSuccess: async (schedule) => {
      ...
      await invalidate();
      toast({
        title: "Draft schedule persisted",
        description: `Schedule #${schedule.id} is replayable and awaiting human approval.`,
      });
    },
    onError: (error) =>
      toast({
        title: "Schedule persistence failed",
        description: messageOf(error),
        variant: "destructive",
      }),
  });
  ...
  const transition = useMutation({
    mutationFn: ({
      ...
      const reason =
        transitionReasons[schedule.id]?.trim()
        || `Household ${eventType} confirmation`;
      const payload = {
    ...
    onSuccess: async (schedule) => {
      ...
      toast({
        ...
        description: `Schedule #${schedule.id} is now version ${schedule.version}.`,
      });
    },
    onError: (error) =>
      toast({
        title: "Schedule transition failed",
        description: messageOf(error),
        variant: "destructive",
      }),
  });
  ...
  const pageError =
    householdsQ.error || detailQ.error || calendarsQ.error || schedulesQ.error;
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Reviewed household resource calendars, deterministic replayable
            schedules, explicit approval, and append-only lifecycle history.
          </p>
        </div>

        {(pageError || handoffError) && (
        ...
        {handoffLoaded && (
          <Alert>
            ...
            <AlertTitle>Reviewed pipeline handoff loaded</AlertTitle>
            <AlertDescription>
              The exact occurrence document, request, and response are prefilled
              below. Inspect them and choose Persist replayable draft; nothing
              has been persisted or approved automatically.
            </AlertDescription>
          </Alert>
        )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Every calendar, schedule, transition, and event remains isolated
              to one authorized household.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-3 md:grid-cols-[1fr_auto_auto] md:items-end">
            ...
            <Badge
              variant={activeCalendar ? "default" : "secondary"}
              className="w-fit"
            >
              {activeCalendar
                ? `Active ${activeCalendar.calendar_version}`
                : "No active reviewed calendar"}
            </Badge>
          </CardContent>
        </Card>
        ...
        <Alert>
          ...
          <AlertDescription>
            Calendars are explicit household declarations, schedules are
            server-replayed plans, and approval is human confirmation.
            NutriFlavorOS does not control appliances, infer presence, verify
            execution, or guarantee food safety.
          </AlertDescription>
        </Alert>
            ...
            <TabsContent value="schedules" className="space-y-4">
                  ...
                  <CardHeader>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <CardDescription>
                          Created {formatDate(schedule.created_at)} · optimistic
                      ...
                      <div className="flex flex-wrap gap-1">
                        ...
                        >
                          {schedule.replay_status ?? "legacy_request_missing"}
                        </Badge>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {schedule.replay_status !== "replayable" && (
                      <Alert variant="destructive">
                        ...
                        <AlertDescription>
                          This legacy row lacks the complete occurrence document
                          or replay input. Recreate it with the exact original
                    ...
                    <div className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5">
                      <div>
                        ...
                        <p>#{schedule.calendar_version_id}</p>
                        <code title={schedule.calendar_content_hash}>
                          {shortHash(schedule.calendar_content_hash)}
                        </code>
                      </div>
                      <div>
                        ...
                        <p>{schedule.occurrence_set_version}</p>
                        <code title={schedule.occurrence_set_hash}>
                          {shortHash(schedule.occurrence_set_hash)}
                        </code>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">
                        ...
                        <code title={schedule.schedule_hash}>
                          {shortHash(schedule.schedule_hash)}
                        </code>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">
                        ...
                        <code title={schedule.schedule_request_hash ?? undefined}>
                          {shortHash(schedule.schedule_request_hash)}
                        </code>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Makespan</p>
                        <p>
                          {schedule.schedule.makespan_minutes} minutes ·{" "}
                          {schedule.schedule.scheduled.length} tasks
                        </p>
                      </div>
                    </div>
                    ...
                    {(schedule.status === "draft"
                      ...
                      && canEdit(role) && (
                        <div className="space-y-2 border-t pt-3">
                          <Label htmlFor={`transition-reason-${schedule.id}`}>
                            Lifecycle reason
                          </Label>
                          <Input
                            id={`transition-reason-${schedule.id}`}
                            value={transitionReasons[schedule.id] ?? ""}
                            onChange={(event) =>
                            ...
                            placeholder="Record the human decision or operational outcome"
                          />
                          <TransitionButtons
                            schedule={schedule}
                            role={role}
                            pending={transition.isPending}
                            onTransition={(eventType) =>
                              transition.mutate({ schedule, eventType })
                            }
                          />
                        </div>
                      )}
                        ...
                        <div className="mt-3 space-y-2" aria-live="polite">
                          ...
                          {(eventsQ.data ?? []).map((event) => (
                            ...
                            >
                              ...
                              <p className="text-xs text-muted-foreground">
                                Actor {event.actor_user_id} · fingerprint{" "}
                                <code title={event.request_fingerprint}>
                                  {shortHash(event.request_fingerprint)}
                                </code>
                              </p>
                            </div>
                          ))}
              ...
              {!schedulesQ.isLoading
                && (schedulesQ.data?.length ?? 0) === 0 && (
                  <p className="text-sm text-muted-foreground">
                    No persisted preparation schedules.
                  </p>
                )}
            </TabsContent>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base">
                      Register and activate a reviewed calendar
                    </CardTitle>
                    <CardDescription>
                      Activating a successor invalidates draft and approved
                      schedules linked to the prior calendar.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <form
                    ...
                    >
                      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
                        <div className="space-y-1">
                          <Label htmlFor="calendar-version">Version</Label>
                          <Input
                            id="calendar-version"
                            value={calendarVersion}
                            onChange={(event) =>
                              setCalendarVersion(event.target.value)
                            }
                            required
                          />
                        </div>
                        <div className="space-y-1">
                          <Label htmlFor="calendar-horizon">
                            Horizon minutes
                          </Label>
                          <Input
                            id="calendar-horizon"
                        ...
                        <div className="space-y-1">
                          <Label htmlFor="calendar-reviewed-at">
                            Reviewed at
                          </Label>
                          <Input
                            id="calendar-reviewed-at"
                            type="datetime-local"
                            value={reviewedAt}
                            onChange={(event) =>
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        <div className="space-y-1">
                          <Label htmlFor="calendar-reviewed-by">
                            Reviewed by
                          </Label>
                          <Input
                            id="calendar-reviewed-by"
                            value={reviewedBy}
                            onChange={(event) =>
                        ...
                        <div className="space-y-1">
                          <Label htmlFor="calendar-notes">Review notes</Label>
                          <Input
                            id="calendar-notes"
                            value={calendarNotes}
                            onChange={(event) =>
                      ...
                      <div className="space-y-1">
                        <Label htmlFor="calendar-resources">
                          Resources and explicit windows JSON
                        </Label>
                        <Textarea
                          ...
                          value={resourcesJson}
                          onChange={(event) =>
                            setResourcesJson(event.target.value)
                          }
                          required
                        />
                      </div>
                      ...
                      >
                        Register active reviewed calendar
                      </Button>
                    </form>
                  </CardContent>
                </Card>
                ...
                {(calendarsQ.data ?? []).map((calendar) => (
                  <Card key={calendar.id}>
                    <CardHeader>
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <div>
                          ...
                          <CardDescription>
                            Record #{calendar.id} · {calendar.timezone} ·{" "}
                            {calendar.horizon_minutes} minutes
                          </CardDescription>
                        </div>
                        <div className="flex gap-1">
                          <Badge variant="outline">
                            {calendar.evidence_status}
                          </Badge>
                          <Badge
                            variant={calendar.active ? "default" : "secondary"}
                          >
                            {calendar.active ? "active" : "historical"}
                          </Badge>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-3 text-sm">
                      <p>
                        Reviewed by {calendar.reviewed_by ?? "not recorded"} ·{" "}
                        {formatDate(calendar.reviewed_at)}
                      </p>
                      <p>
                        SHA-256{" "}
                        <code title={calendar.content_hash}>
                          {shortHash(calendar.content_hash)}
                        </code>
                      </p>
                      {calendar.supersedes_calendar_id && (
                        <p className="text-muted-foreground">
                          Supersedes calendar #{calendar.supersedes_calendar_id}
                        </p>
                      )}
            ...
            <TabsContent value="ingest">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base">
                    <CheckCircle2 className="h-4 w-4" />
                    Persist reviewed deterministic output
                  </CardTitle>
                  <CardDescription>
                    Inspect the complete occurrence document, request, and
                    response. The server rejects household/resource mismatch,
                    stale plan, unresolved work, provenance drift, or replay
                    difference.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  ...
                  >
                    <div className="space-y-1">
                      <Label htmlFor="schedule-bundle">
                        Schedule creation bundle JSON
                      </Label>
                      <Textarea
                        id="schedule-bundle"
                        ...
                        onChange={(event) => {
                          setHandoffLoaded(false);
                          setScheduleJson(event.target.value);
                        }}
                        required
                      />
                    </div>
                    <Button
                      type="submit"
                      disabled={!canEdit(role) || createSchedule.isPending}
                    >
