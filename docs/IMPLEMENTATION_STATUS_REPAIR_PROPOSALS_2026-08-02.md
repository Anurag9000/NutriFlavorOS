...
**Superseded by:** migration `20260802_0018`, API `0.15.2`, OpenAPI `2026-08-02.12`
...
At the proposal-only boundary, NutriFlavorOS had advisory deterministic minimal-change repair, immutable server-recomputed proposals, exact source/calendar/provenance/repair hashes, complete-only proposal persistence, exact creation/rejection idempotency, append-only events, read-time staleness/tamper detection, source-execution abstention, protected review interfaces, and permanent non-acceptance/non-persistence computation fields.
...
- owner-only proposal invalidation with server-observed stale reasons;
- typed invalidation client and strict role/idempotency contracts;
...
6. An owner may invalidate only a still-proposed record; invalidation creates no schedule and prevents later acceptance.
7. Task execution remains separate.
8. A replaced source remains readable but cannot receive new execution events or completion.
9. Schedule completion remains guarded by explicit task terminality through the product endpoint.
...
- Invalidation never accepts, persists, approves, executes, completes, or mutates the source.
